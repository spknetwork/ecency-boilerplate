...
export const fetchEntries =
  ...
  (dispatch: Dispatch, getState: () => AppState) => {
    ...
    let theEntries = entries[groupKey]?.entries || [];
    ...
    if (Array.isArray(tag)) {
      promises = tag.map((currentTag) => {
        let promise: Promise<Entry[]>;
        if (currentTag.startsWith("@")) {
          const username = currentTag.replace("@", "");
    ...
    Promise.all(promises)
      .then((responses) => {
        const flatResponses = responses.flat(); // Flatten the array
        dispatch(
          fetchedAct(groupKey, flatResponses, flatResponses.length >= dataLimit)
        );
      })
      .catch(() => {
        dispatch(fetchErrorAct(groupKey, "error"));
      });
  };
